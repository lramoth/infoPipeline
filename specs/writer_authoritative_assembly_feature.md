...
Prevent model-generated text from changing curator-provided titles, source URLs, or item ordering while keeping the Writer prompt flexible for future presentation changes.
...
The Writer prompt may change over time to alter tone, format, length, or presentation. Preserving curator-provided titles, URLs, and order must not depend on parsing a model-generated final briefing layout.
...
3. Use the configured local model to generate concise prose for the curated items.
4. Produce a final outbound message containing all curated items.
5. Ensure the final outbound message uses the exact title and exact source URL from each corresponding Curator item.
6. Ensure the final outbound message presents items in ascending rank order.
7. Validate the final outbound message before returning it.
...
- Given a Writer prompt that changes final-message presentation wording, headings, bullets, source-label wording, or markdown style, the final outbound message still preserves curator-provided titles, source URLs, and rank order.
